...
You can permanently set an option by putting it in the `~/.crarc` file.  The
...
location `$XDG_CONFIG_HOME/cra/config`.  For brevity, this document will
refer only to `~/.crarc`.

Arc configuration
-----------------

Alternatively to using `~/.crarc`, Cra options can be set by putting them in
one of the Arc configuration files, which are read by Cra on startup. See
'arc-config(1)' for which files to use. The following example shows the basic
syntax to use for settings, bindings and colors.

// TEST: gitconfig
--------------------------------------------------------------------------
[cra] show-changes = true
[cra "color"] cursor = yellow red bold
[cra "bind"] generic = P parent
--------------------------------------------------------------------------

In addition to cra-specific options, the following Arc options are read from
the Arc configuration:

'color.*'::

	Colors for the various UI types. Can be configured via the 'arc-colors'
	setting.
...
'core.editor'::
	...
	The editor command. Can be overridden by setting CRA_EDITOR or
	ARC_EDITOR.
...
'gui.encoding'::
	...
	The encoding to use for displaying file content.
...
Use *+=* to append value to a string variable.
...
set arc-colors = no		# Do not read Arc's color settings.
set horizontal-scroll = 33%	# Scroll 33% of the view width
set blame-options = -C -C -C	# Blame lines from other files
set log-options += --minimal  	# Use minimal diff algorithm
...
Or in the Arc configuration files:
...
'diff-options' (string)::
	...
	arc-show(1) is used for formatting and always passes --patch-with-stat.
	Can control display of commit header metadata, passing option --format.
	This option overrides any options specified in the CRA_DIFF_OPTS
	environment variable (described in manpage:cra[1]), but is itself
...
'blame-options' (string)::
	...
	telling arc-blame(1) how to detect the origin of lines. The options
	are ignored when Cra is started in blame mode and given blame options
	on the command line.

'log-options' (string)::

	A space-separated string of default options that should be passed to the
	arc-log(1) command used by the log view. Options can be overridden by
	command line options. Used internally to override custom `pretty.format'
	settings that break the log view.

'main-options' (string)::

	A space-separated string of default options that should be passed to the
	arc-log(1) command used by the main view. Options can be overridden by
...
'reference-format' (string)::
	...
	Supported reference types are:
	 ...
	 - stash		: The stash.
	 - note			: Notes.
	 - prefetch		: Refs prefetched by `arc maintenance`.
...
'arc-colors' (list)::
	...
	A space-separated list of "key=value" pairs where the key is a Arc color
	name and the value is a Cra color name, e.g. "branch.current=main-head"
...
'show-notes' (mixed) [<reference>|<bool>]::
	...
	reference is passed to `arc show --notes=`. Notes are enabled by
	default.
...
'show-untracked' (bool)::
	...
	Whether to also show untracked changes in the main view.
...
'status-show-untracked-dirs' (bool)::
	...
	Show untracked directory contents in the status view (analogous to
	`arc ls-files --directory` option). On by default.

'status-show-untracked-files' (bool)::

	Show untracked files in the status view (mirrors Arc's
...
'diff-highlight' (mixed)::
	...
	Whether to highlight diffs using Arc's 'diff-highlight' program. Defaults
...
'ignore-space' (mixed) [no|all|some|at-eol|<bool>]::
	...
	"--ignore-space-at-eol" respectively to `arc diff` or `arc show`. +
	 +
...
'commit-order' (enum) [auto|default|topo|date|author-date|reverse]::
	...
	Commit ordering using the default (reverse chronological) order,
...
'mailmap' (bool)::
	...
	`.mailmap`. Off by default. See `arc-shortlog(1)`.
...
'history-size' (int)::
	...
	Size of the persistent `~/.cra_history` file when compiled with readline
...
'mouse-scroll' (int)::
	...
	you have enabled mouse support in ~/.crarc with `set mouse = true`.
...
'pgrp' (bool)::
	...
	Make cra the process-group leader when starting and clean all processes
	...
	`setsid` to keep clipboard content after exiting cra. If you are using
	arc-credential-cache helper, set option `credentialCache.ignoreSIGHUP`.
...
Column specifications start with the column type, and can optionally be
...
the defaults in the system crarc file. To override a single column, use the
column name as a suffix after the view setting name, e.g. `main-view-date` will
allow setting the date in the main view.
...
# Change grep view to be similar to `arc grep` format
...
# Use Arc's default commit order, even when the commit graph is enabled.
set commit-order = default
--------------------------------------------------------------------------

The following list shows the available view settings and what column types
they support:
...
commit-title::
	- 'graph' (mixed) [no|v2|v1]: Whether to show the revision graph in the
	  ...
	  repositories.  See also the 'line-graphics' option.
	...
	- 'overflow' (bool or int): Whether to highlight text in commit titles
	  ...
	  the highlighting using the default width of 50 characters. When set to
...
id::
	...
	- 'width' (int): Fixed width for the commit ID column. When unset Cra
	  will use the value of 'core.abbrev' if found. See arc-config(1) on how
...
text::
	- 'commit-title-overflow' (bool or int): Whether to highlight commit
	  ...
	  width of 50 characters. When set to an int, the assigned value is used
...
# Disable the default mapping for running arc-gc
bind generic G none

# User-defined external command to amend the last commit
bind status + !arc commit --amend

# User-defined internal command that reloads ~/.crarc
bind generic S :source ~/.crarc
...
Cra respects but does not require an `$if cra` section in `~/.inputrc`.
...
insensitive and start with "<" and end with ">". Use *<Hash>* to bind to the
...
- Cra reads keystrokes via ncurses and is subject to various limitations. See
...
- `Ctrl-z` is automatically used for process control and will suspend Cra and
  open a subshell (use `fg` to reenter Cra).
...
|<			|Exit Cra after executing the command.
|>			|Re-open Cra instantly in the last displayed view after
...
are specified their behavior is combined, e.g. "?<arc commit" will prompt the
user whether to execute the command and will exit Cra after completion.
...
User-defined commands can optionally refer to Cra's internal state using the
...
bind main S !arc format-patch -1 %(commit)

# Create and checkout a new branch; specify custom prompt
bind main B ?arc checkout -b "%(prompt Enter new branch name: )"

# Show commit statistics for the author under the cursor
bind main U +sh -c 'arc --no-pager shortlog -s --author="$(arc show -s --format=%aE %(commit))" </dev/tty'
--------------------------------------------------------------------------
...
Or by using a combination of Arc aliases and Cra external commands. The
following example entries can be put in either the .gitconfig or .arc/config
file:

.Arc configuration which binds Cra keys to Arc command aliases.
// TEST: gitconfig
--------------------------------------------------------------------------
[alias]
	gitk-bg = !"gitk HEAD --not $(arc rev-parse --remotes) &"
	publish = !"for i in origin public; do arc push $i; done"
[cra "bind"]
	generic = V @arc gitk-bg
	generic = > !arc publish
--------------------------------------------------------------------------
...
|main-prefetch		|Label of reference prefetched by `arc maintenance`.
...
Enhanced Arc diff markup::

Extra diff information emitted by the Arc diff machinery, such as mode
...
If the given path does not exist, cra will proceed with a warning. Give
...
// TEST: tigrc
--------------------------------------------------------------------------
source ~/.cra/colorscheme.crarc
source ~/.cra/keybindings.crarc
--------------------------------------------------------------------------
